...
function RegisterPageContent() {
  ...
  useEffect(() => {
    ...
    if (saved) {try{ setInitialData(JSON.parse(saved)); } catch{}}
  }, []);
  ...
  const handleSubmit = async (formData: any) => {
    try {
      ...
      toast({
        ...
        description: 'Bienvenue sur ParkHub !',
      });
      router.push('/home');
    } catch (error) {
      toast({
        ...
        description: 'Une erreur est survenue à l\'inscription. Veuillez réessayez.',
      });
    }
  };
  ...
  return (
    ...
    >
      ...
      <Card className={`${isMobile ? 'p-5' : 'p-6 md:p-8'} space-y-6`}>
        <RegisterStepper onSubmit={handleSubmit} initialData={initialData} />
