...
export function AddUserForm({ onSubmit, onCancel }: AddUserFormProps) {
  const [formData, setFormData] = useState({ firstName: '', lastName: '', email: '', phone: '', password: '', vehiclePlates: [''] });
  ...
  const addPlate = () => setFormData(prev => ({ ...prev, vehiclePlates: [...prev.vehiclePlates, ''] }));
  const removePlate = (idx: number) => setFormData(prev => ({ ...prev, vehiclePlates: prev.vehiclePlates.filter((_, i) => i !== idx) }));
      ...
      <div className="space-y-2">
        ...
        {formData.vehiclePlates.map((plate, idx) => (
          <div key={idx} className="flex gap-2 items-start">
            <Input value={plate} onChange={(e) => updatePlate(idx, e.target.value)} error={errors[`plate_${idx}`]} placeholder="ex: AB123CD" className="flex-1" />
            {idx > 0 && <Button type="button" variant="secondary" onClick={() => removePlate(idx)}><X className="w-4 h-4" /></Button>}
          </div>
        ))}
